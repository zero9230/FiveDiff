...
IMPLEMENT_DYNAMIC(CQcXTestView, CDialogEx)
//extern  qcresult_info	 qcresultdata;			//与界面交互和数据库交互的变量
extern  patient_info  patientdata;

unsigned int CQcXTestView::QcXFileExistEdit = 1;	//0:文件没有编辑信息，1:文件有编辑信

extern CQcXListView			m_QcXListPage;
extern CQcXChartView		m_QcXChartPage;
CQcXTestView::CQcXTestView(CWnd* pParent /*=NULL*/)
	: CDialogEx(CQcXTestView::IDD, pParent)
{
...
CQcXTestView::~CQcXTestView()
{
}

void CQcXTestView::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_QC_X_WBCLIST, m_WBCList);
	DDX_Control(pDX, IDC_QC_X_RBCLIST, m_RBCList);
	DDX_Control(pDX, IDC_QC_X_PLTLIST, m_PLTList);
	DDX_Control(pDX, IDC_QC_X_RETLIST, m_RETList);
	DDX_Control(pDX, IDC_QC_X_CRPLIST, m_CRPList);
	DDX_Control(pDX, IDC_QC_X_LMNE_CHART, m_LmneChart);
	DDX_Control(pDX, IDC_QC_X_BASO_CHART, m_BASOChart);
	DDX_Control(pDX, IDC_QC_X_RBC_CHART, m_RBCChart);
	DDX_Control(pDX, IDC_QC_X_PLT_CHART, m_PLTChart);
	DDX_Control(pDX, IDC_QC_X_CHART4, m_PieChart);
}


BEGIN_MESSAGE_MAP(CQcXTestView, CDialogEx)
	ON_WM_PAINT()
	ON_WM_CTLCOLOR()
	ON_WM_ERASEBKGND()
	ON_MESSAGE(WM_QUALITYTEST, &CQcXTestView::OnQualitytest)
	ON_WM_TIMER()
	ON_MESSAGE(WM_ACKSPI, &CQcXTestView::OnAckspi)
END_MESSAGE_MAP()

// CQcXTestView 消息处理程序


void CQcXTestView::OnPaint()
{
...
HBRUSH CQcXTestView::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CQcXTestView::OnInitDialog()
{
	...
	//getqcresultRow();
	//item_temp.Format(L"zhizhizhizhi");
	SetDlgItemText(IDC_QC_X_STATIC0, item_temp);
	GetDlgItem(IDC_QC_X_STATIC0)->SetFont(&textfont);

	SetDlgItemText(IDC_QC_X_STATIC1, L"第一组： ");
	SetDlgItemText(IDC_QC_X_STATIC2, L"第二组： ");
	SetDlgItemText(IDC_QC_X_STATIC_AVE, L"平均值： ");
...
BOOL CQcXTestView::InitForm(qcresult_info *qcresultdata)
{
...
BOOL CQcXTestView::UpdateForm(qcresult_info *qcresultdata)
{
...
void CQcXTestView::PrintChart(qcresult_info *qcresultdata)
{
	//PrintLMNEChart(qcresultdata);
...
void CQcXTestView::UpdatePrintChart(qcresult_info *qcresultdata)
{
	UpdateData();
	OnPaint();
...
BOOL CQcXTestView::InitWBCForm(qcresult_info *qcresultdata)
{
	CRect rect;
...
BOOL CQcXTestView::UpdateWBCForm(qcresult_info *qcresultdata)
{
	/*
...
BOOL CQcXTestView::InitRBCForm(qcresult_info *qcresultdata)
{
	CRect rect;
...
BOOL CQcXTestView::UpdateRBCForm(qcresult_info *qcresultdata)
{
...
BOOL CQcXTestView::InitPLTForm(qcresult_info *qcresultdata)
{
	CRect rect;
...
BOOL CQcXTestView::UpdatePLTForm(qcresult_info *qcresultdata)
{
...
BOOL CQcXTestView::UpdateRETForm()
{
	return TRUE;
}
BOOL CQcXTestView::InitCRPForm()
{
	CRect rect;
...
BOOL CQcXTestView::UpdateCRPForm()
{
	return TRUE;
}
...
void CQcXTestView::Init_B_LMNE(float	coefficient, B_LMNE* p_blmne)
{
	float		coe_rtoabs;
...
************************************************************/
void CQcXTestView::LmneGraphPaint(qcresult_info *qcresultdata, const unsigned char rangetype)
{
	unsigned int i;
	...
	B_LMNE		m_blmne;
	CRect rect;
	CWnd* pWnd = GetDlgItem(IDC_QC_X_LMNE_CHART);
...
//绘制lmne内外框线
void CQcXTestView::PrintLMNEChart(qcresult_info *qcresultdata)
{
	CRect rect;
	CWnd* pWnd = GetDlgItem(IDC_QC_X_LMNE_CHART);
	pWnd->GetClientRect(&rect); // 获取控件屏幕坐标
	...
	LmneGraphPaint(qcresultdata, patientdata.rangetype);
	//GetDlgItem(IDC_QC_X_LMNE_CHART)->EnableWindow(FALSE);
}

void CQcXTestView::PrintBASOChart(qcresult_info *qcresultdata)
{
...
void CQcXTestView::UpdatePrintBASOChart(qcresult_info *qcresultdata)
{
	/*
	UpdateData();
	unsigned short int i;
...
void CQcXTestView::PrintRBCChart(qcresult_info *qcresultdata)
{
	unsigned short int i;
	unsigned char graphbuff[256] = { 0 };
...
void CQcXTestView::UpdatePrintRBCChart(qcresult_info *qcresultdata)
{
	/*
	UpdateData();
	unsigned short int i;
...
void CQcXTestView::PrintPLTChart(qcresult_info *qcresultdata)
{
	/*	unsigned short int i;
	unsigned char graphbuff[256] = { 0 };
...
void CQcXTestView::UpdatePrintPLTChart(qcresult_info *qcresultdata)
{
	/*
	UpdateData();
	unsigned short int i;
...
void CQcXTestView::PrintPieChart()
{
	unsigned short int i;
...
void CQcXTestView::UpdatePrintPieChart()
{
	UpdateData();
	m_PieChart.EnableTooltip();
...
BOOL CQcXTestView::OnEraseBkgnd(CDC* pDC)
{
...
void CQcXTestView::OnTimer(UINT_PTR nIDEvent)
{
	// TODO:  在此添加消息处理程序代码和/或调用默认值
	switch (nIDEvent){
	case SLEEP_TIMER1:
...
void CQcXTestView::initQcXData()//质控测试参数就是26个，没有必要28个，和主测试流程不一样
{
	unsigned int i = 0;
...
afx_msg LRESULT CQcXTestView::OnAckspi(WPARAM wParam, LPARAM lParam)
{
	int rowCount;
	switch (wParam)
	{
	case SPI_STATE_DATAISOK:
		...
		if (1 == testNum)
		{
			Addqcresult1ToAve();	//将第一次测试结果放到平均测试结果中
			//GetDlgItem(IDC_QC_X_STATIC1)->SetDlgItemText(" ");
			SetDlgItemText(IDC_QC_X_STATIC1, L"第一组：已做");
			SetDlgItemText(IDC_QC_X_STATIC2, L"第二组： ");
			SetDlgItemText(IDC_QC_X_STATIC_AVE, L"平均值： ");
		}
		else if (2 == testNum)
		{
			Addqcresult2ToAve();	//将第二次测试结果放到平均测试结果中
			//数据库保存	
			rowCount=getqcresultRow();
			if (rowCount > 31)//将最前面的一个记录删除
				DelQcLjXXrResult(Controltype, Controlfile, qcresultdataAve.row - 31);
			AddQcLjXXrResult(&qcresultdataAve);
			SetDlgItemText(IDC_QC_X_STATIC1, L"第一组：已做");
			SetDlgItemText(IDC_QC_X_STATIC2, L"第二组：已做");
			SetDlgItemText(IDC_QC_X_STATIC_AVE, L"平均值：已做");

			//刷新质控的其他页面——质控图、质控列表		
			::PostMessage(m_QcXChartPage, UM_REDRAW, 0, 0);
			::PostMessage(m_QcXListPage, UM_REDRAW, 0, 0);
		}
...
//获得最后记录数
int CQcXTestView::getqcresultRow()
{
	CString filename;
	CString	qctypetemp;
...
void CQcXTestView::initQcXAveData()//质控测试参数就是26个，没有必要28个，和主测试流程不一样
{
	unsigned int i = 0;
...
//将第一次测试结果放到平均测试结果中
void CQcXTestView::Addqcresult1ToAve()
{
	strcpy(qcresultdataAve.wbcdata.wbc, qcresultdata.wbcdata.wbc);

	strcpy(qcresultdataAve.wbcdata.lymp, qcresultdata.wbcdata.lymp);
	strcpy(qcresultdataAve.wbcdata.neup, qcresultdata.wbcdata.neup);
	strcpy(qcresultdataAve.wbcdata.monop, qcresultdata.wbcdata.monop);
	strcpy(qcresultdataAve.wbcdata.eosp, qcresultdata.wbcdata.eosp);
	strcpy(qcresultdataAve.wbcdata.basp, qcresultdata.wbcdata.basp);
	strcpy(qcresultdataAve.wbcdata.alyp, qcresultdata.wbcdata.alyp);
	strcpy(qcresultdataAve.wbcdata.licp, qcresultdata.wbcdata.licp);

	strcpy(qcresultdataAve.wbcdata.lym, qcresultdata.wbcdata.lym);
	strcpy(qcresultdataAve.wbcdata.neu, qcresultdata.wbcdata.neu);
	strcpy(qcresultdataAve.wbcdata.mono, qcresultdata.wbcdata.mono);
	strcpy(qcresultdataAve.wbcdata.eos, qcresultdata.wbcdata.eos);
	strcpy(qcresultdataAve.wbcdata.bas, qcresultdata.wbcdata.bas);
	strcpy(qcresultdataAve.wbcdata.aly, qcresultdata.wbcdata.aly);
	strcpy(qcresultdataAve.wbcdata.lic, qcresultdata.wbcdata.lic);
	//RBC
	strcpy(qcresultdataAve.rbcdata.rbc, qcresultdata.rbcdata.rbc);
	strcpy(qcresultdataAve.rbcdata.hgb, qcresultdata.rbcdata.hgb);
	strcpy(qcresultdataAve.rbcdata.hct, qcresultdata.rbcdata.hct);
	strcpy(qcresultdataAve.rbcdata.mcv, qcresultdata.rbcdata.mcv);
	strcpy(qcresultdataAve.rbcdata.mch, qcresultdata.rbcdata.mch);
	strcpy(qcresultdataAve.rbcdata.mchc, qcresultdata.rbcdata.mchc);
	strcpy(qcresultdataAve.rbcdata.rdw, qcresultdata.rbcdata.rdw);
	//PLT
	strcpy(qcresultdataAve.pltdata.plt, qcresultdata.pltdata.plt);
	strcpy(qcresultdataAve.pltdata.mpv, qcresultdata.pltdata.mpv);
	strcpy(qcresultdataAve.pltdata.pdw, qcresultdata.pltdata.pdw);
	strcpy(qcresultdataAve.pltdata.pct, qcresultdata.pltdata.pct);
}

//将第二次测试结果放到平均测试结果中，并取平均值
void CQcXTestView::Addqcresult2ToAve()
{
	unsigned int i = 0;
	float tempValue;
	...
	tempValue = atof(qcresultdataAve.wbcdata.wbc) + atof(qcresultdata.wbcdata.wbc);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.wbc, "%.2f", tempValue);

	tempValue = atof(qcresultdataAve.wbcdata.lymp) + atof(qcresultdata.wbcdata.lymp);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.lymp, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.neup) + atof(qcresultdata.wbcdata.neup);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.neup, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.monop) + atof(qcresultdata.wbcdata.monop);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.monop, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.eosp) + atof(qcresultdata.wbcdata.eosp);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.eosp, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.basp) + atof(qcresultdata.wbcdata.basp);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.basp, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.alyp) + atof(qcresultdata.wbcdata.alyp);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.alyp, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.licp) + atof(qcresultdata.wbcdata.licp);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.licp, "%.1f", tempValue);

	tempValue = atof(qcresultdataAve.wbcdata.lym) + atof(qcresultdata.wbcdata.lym);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.lym, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.neu) + atof(qcresultdata.wbcdata.neu);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.neu, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.mono) + atof(qcresultdata.wbcdata.mono);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.mono, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.eos) + atof(qcresultdata.wbcdata.eos);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.eos, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.bas) + atof(qcresultdata.wbcdata.bas);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.bas, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.aly) + atof(qcresultdata.wbcdata.aly);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.aly, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.wbcdata.lic) + atof(qcresultdata.wbcdata.lic);
	tempValue /= 2;
	sprintf(qcresultdataAve.wbcdata.lic, "%.2f", tempValue);
	//RBC
	tempValue = atof(qcresultdataAve.rbcdata.rbc) + atof(qcresultdata.rbcdata.rbc);
	tempValue /= 2;
	sprintf(qcresultdataAve.rbcdata.rbc, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.rbcdata.hgb) + atof(qcresultdata.rbcdata.hgb);
	tempValue /= 2;
	sprintf(qcresultdataAve.rbcdata.hgb, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.rbcdata.hct) + atof(qcresultdata.rbcdata.hct);
	tempValue /= 2;
	sprintf(qcresultdataAve.rbcdata.hct, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.rbcdata.mcv) + atof(qcresultdata.rbcdata.mcv);
	tempValue /= 2;
	sprintf(qcresultdataAve.rbcdata.mcv, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.rbcdata.mch) + atof(qcresultdata.rbcdata.mch);
	tempValue /= 2;
	sprintf(qcresultdataAve.rbcdata.mch, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.rbcdata.mchc) + atof(qcresultdata.rbcdata.mchc);
	tempValue /= 2;
	sprintf(qcresultdataAve.rbcdata.mchc, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.rbcdata.rdw) + atof(qcresultdata.rbcdata.rdw);
	tempValue /= 2;
	sprintf(qcresultdataAve.rbcdata.rdw, "%.1f", tempValue);
	//PLT
	tempValue = atof(qcresultdataAve.pltdata.plt) + atof(qcresultdata.pltdata.plt);
	tempValue /= 2;
	sprintf(qcresultdataAve.pltdata.plt, "%.0f", tempValue);
	tempValue = atof(qcresultdataAve.pltdata.mpv) + atof(qcresultdata.pltdata.mpv);
	tempValue /= 2;
	sprintf(qcresultdataAve.pltdata.mpv, "%.2f", tempValue);
	tempValue = atof(qcresultdataAve.pltdata.pdw) + atof(qcresultdata.pltdata.pdw);
	tempValue /= 2;
	sprintf(qcresultdataAve.pltdata.pdw, "%.1f", tempValue);
	tempValue = atof(qcresultdataAve.pltdata.pct) + atof(qcresultdata.pltdata.pct);
	tempValue /= 2;
