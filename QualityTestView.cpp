...
extern  sample_info	 sampledata;			//与界面交互和数据库交互的变量
extern  patient_info  patientdata;

unsigned char CQualityTestView::QcLjFileExistEdit;	//0:文件没有编辑信息，1:文件有编辑信
...
BOOL CQualityTestView::InitForm(sample_info* psampledata)
{
...
BOOL CQualityTestView::UpdateForm(sample_info* psampledata)
{
...
void CQualityTestView::PrintChart(sample_info *psampledata)
{
...
void CQualityTestView::UpdatePrintChart(sample_info *psampledata)
{
	UpdateData();
	OnPaint();
...
BOOL CQualityTestView::InitWBCForm(sample_info* psampledata)
{
	CRect rect;
...
BOOL CQualityTestView::UpdateWBCForm(sample_info* psampledata)
{
...
BOOL CQualityTestView::InitRBCForm(sample_info* psampledata)
{
	CRect rect;
...
BOOL CQualityTestView::UpdateRBCForm(sample_info* psampledata)
{
...
BOOL CQualityTestView::InitPLTForm(sample_info* psampledata)
{
	CRect rect;
...
BOOL CQualityTestView::UpdatePLTForm(sample_info* psampledata)
{
...
************************************************************/
void CQualityTestView::LmneGraphPaint(sample_info *psampledata, const unsigned char rangetype)
{
	unsigned int i;
...
void CQualityTestView::PrintLMNEChart(sample_info *psampledata)
{
	CRect rect;
	CWnd* pWnd = GetDlgItem(IDC_CHART);
	pWnd->GetClientRect(&rect); // 获取控件屏幕坐标
...
void CQualityTestView::PrintBASOChart(sample_info *psampledata)
{
...
void CQualityTestView::UpdatePrintBASOChart(sample_info *psampledata)
{
	UpdateData();
	unsigned short int i;
	unsigned char graphbuff[256] = { 0 };
...
void CQualityTestView::PrintRBCChart(sample_info *psampledata)
{
	unsigned short int i;
	unsigned char graphbuff[256] = { 0 };
...
void CQualityTestView::UpdatePrintRBCChart(sample_info *psampledata)
{
	UpdateData();
	unsigned short int i;
	unsigned char graphbuff[256] = { 0 };
...
void CQualityTestView::PrintPLTChart(sample_info *psampledata)
{
	unsigned short int i;
	unsigned char graphbuff[256] = { 0 };
...
void CQualityTestView::UpdatePrintPLTChart(sample_info *psampledata)
{
	UpdateData();
	unsigned short int i;
	unsigned char graphbuff[256] = { 0 };
...
void CQualityTestView::initQcLjData()//质控测试参数就是26个，没有必要28个，和主测试流程不一样
{
	...
	qcresultdata.qctype = Controltype;
	qcresultdata.filenum = Controlfile;
	qcresultdata.time = 0;
