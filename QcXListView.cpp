...
IMPLEMENT_DYNAMIC(CQcXListView, CDialogEx)

CQcXListView::CQcXListView(CWnd* pParent /*=NULL*/)
: CDialogEx(CQcXListView::IDD, pParent)
, pageNum(_T(""))
{
	for (int i = 0; i < 26; i++)
	{
...
CQcXListView::~CQcXListView()
{
}

void CQcXListView::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_QC_X_LIST_LIST, m_ListList);
	DDX_Text(pDX, IDC_QC_X_LIST_PAGE_NUM, pageNum);
}


BEGIN_MESSAGE_MAP(CQcXListView, CDialogEx)
	...
	ON_BN_CLICKED(IDC_PRINT_X, &CQcXListView::OnBnClickedPrintX)
END_MESSAGE_MAP()
...
BOOL CQcXListView::OnInitDialog()
{
	...
	SetDlgItemText(IDC_QC_X_LIST_STATIC0, item_temp);
	GetDlgItem(IDC_QC_X_LIST_STATIC0)->SetFont(&textfont);
...
void CQcXListView::InitListList()
{
	/***************初始化表格**************/
	CRect rect;
...
//获取靶值和偏差限,并填入列表中
bool CQcXListView::GetQcXEditData(){
	CString filename;
	CString	qctypetemp;
...
//获取质控结果
int CQcXListView::GetQcXResultData(int controltype, int controlfile){
...
void CQcXListView::UpdateListResultList(){
...
void CQcXListView::OnBnClickedQcXListUpButton()
{
	// TODO:  在此添加控件通知处理程序代码
...
void CQcXListView::OnBnClickedQcXListDownButton()
{
	// TODO:  在此添加控件通知处理程序代码
	if (curpage <maxPage)
	{
...
void CQcXListView::OnBnClickedPrintX()
{
	CString table[28] = { "日期", "时间", "WBC", "LYM%", "NEU%", "MONO%", "EOS%", "BASO", "ALY%", "LIC%", "LYM#", "NEU#", "MONO#", "EOS#", "BASO#", "ALY#",
		"LIC#", "RBC", "HGB", "HCT", "MCV", "MCH", "MCHC", "RDW", "PLT", "MPV", "PDW", "PCT" };
	int page_num;//打印的页数
	...
	if (PrintDlg(&pd) == TRUE)
	{
		...
		{
			StartPage(pd.hDC);
			pDC.TextOutW(2200, 300, L"X靶值编辑(文件03)");
			if (m_page == 0)
